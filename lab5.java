...
public class lab5 {
    ...
    public static void main(String[] args)throws FileNotFoundException, IOException {
        ...
        char[] chars = new char[ghr_size];
        Arrays.fill(chars, '0');
        String ghr = new String(chars);

        String cell = "00";
        String[] pred_table = new String[(int)Math.pow(2,ghr_size)];
        Arrays.fill(pred_table, cell);
        // lab 5 end
        ...
        while (!input[0].equals("q")){
            ...
            if (input[0].equals("s")){
                 ...
                 while (cnt != iter && currPos < instructionArray.size()){
                     ...
                     if (currCmd.get(0).equals("add")){
                        ...
                        String offset = (String) currCmd.get(3);
                         ...
                         int index = Integer.parseInt(ghr, 2);
                         String pred = pred_table[index];
                         ...
                         if (registers.get(readReg1) == registers.get(readReg2)) {
                             ...
                             // check pred
                             if (pred.charAt(0) == '1') correct_predictions++;

                             // update table
                             if (pred.equals("00")) {
                                 pred_table[index] = "01";
                             } else if (pred.equals("01")) {
                                 pred_table[index] = "10";
                             } else if (pred.equals("10")) {
                                 pred_table[index] = "11";
                             }

                             String temp = ghr.substring(1);
                             ghr = temp + "1";
                         }
                         else{
                             if (pred.charAt(0) == '0') correct_predictions++;
                ...
                for (int csv_line_count = 0; csv_line_count < (dataMem.length / 2); csv_line_count+=2){
                    ...
                    if (dataMem[csv_line_count] == 0 && dataMem[csv_line_count + 1] == 0) break;
                ...
                double acc = (correct_predictions / (double) total_predictions) * 100;

                System.out.println("\naccuracy " + String.format("%.2f", acc) + "% (" + correct_predictions + " correct predictions, " + total_predictions + " predictions)\n");
            }
